...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user && ($user['email_confirmed'] ?? false)) {
        $token = generate_token('password_reset', $user['username'], 1);
        mail_reset_password($user['email'], $user['full_name'], $token);
    }
    $sent = true;
}
?>
...
<title>Forgot Password — FXPulse</title>
<style>
...
.back{display:block;text-align:center;margin-top:20px;font-size:.85em;color:#8b949e;text-decoration:none}
...
<div class="box">
  ...
  <p class="sub">Enter the email address on your account and we'll send a reset link.</p>

  <?php if ($sent): ?>
    <div class="success">&#10003; If that email is registered, a reset link is on its way. Check your inbox (and spam folder). Link expires in 1 hour.</div>
  ...
  <?php else: ?>
    ...
    <p class="back"><a href="login.php">&larr; Back to sign in</a> &nbsp;·&nbsp; <a href="forgot_username.php">Forgot username?</a></p>
  <?php endif; ?>
</div>
</body>
</html>
